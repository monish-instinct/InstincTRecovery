// The HOST for a paired XWatch: connect to its stored remote id, drive
// [XWatchAdapter] over the link, bank what comes back, disconnect.
//
// NOTHING HERE HAS MET HARDWARE (ASSUMPTIONS R6). The registry entry stays
// EXPERIMENTAL, `XWatchAdapter.signals` stays `const {}`, and nothing this
...
import 'adapters/xwatch.dart';
import 'ble_state.dart' show withSecondaryLinkSlot;

String _hex(List<int> b) =>
    b.map((x) => x.toRadixString(16).padLeft(2, '0')).join();

/// The live link to a paired XWatch. One instance; a second concurrent one is
/// not a thing anyone asked for.
class XWatchLink {
  XWatchLink._();
  static final XWatchLink instance = XWatchLink._();
  ...
  static Future<Map<String, Object?>?> pairedRow() async {
    for (final r in await LocalDb.deviceRows()) {
      if (r['adapter_id'] == kXWatch.id) return r;
    }
    return null;
  }
  ...
  Future<bool> _sync() async {
    ...
    // of failure a caller with no try/catch of its own (`_syncXWatch`)
    ...
    try {
      ...
      if (deviceId == LocalDb.kPrimaryDeviceId) {
        debugPrint('[xwatch] refusing to sync: the row claims the primary '
      ...
      return await withSecondaryLinkSlot(() async {
        ...
        try {
          ...
          final link = GattBandLink(
            entry: kXWatch,
            services: services,
            onLog: (m) => debugPrint('[xwatch] $m'),
          );
          _link = link;
          final missing =
              link.missingCharacteristics(kXWatch.requiredCharacteristics);
          if (missing.isNotEmpty) {
            debugPrint('[xwatch] ${kXWatch.label}: missing required '
                'characteristic(s) '
                '${missing.map((u) => u.substring(0, 8)).join(", ")}.');
            return false;
          }
          final host = BandHost(
            adapter: const XWatchAdapter(),
            deviceId: deviceId,
            onLog: (m) => debugPrint('[xwatch] $m'),
      ...
      debugPrint('[xwatch] sync failed: $e');
      return false;
    }
  }
  ...
  /// zero (see `xwatch.dart`'s own header).
