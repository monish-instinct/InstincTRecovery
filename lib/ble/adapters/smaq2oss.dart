// The SMA-Q2-OSS, an open-hardware smartwatch, as a [BandAdapter]:
// subscribe, archive every frame, decode nothing.
//
// NOTHING HERE HAS MET HARDWARE (ASSUMPTIONS R6). Nobody on this project owns
// one, so `signals` is `const {}` and `smaq2oss` is absent from
...
// command to its write characteristic and reading the reply back on the
// separate notify characteristic. There is no captured reply here to check
...
class Smaq2ossAdapter extends BandAdapter {
  const Smaq2ossAdapter();

  @override
  BandEntry get entry => kSmaq2oss;
  ...
  Stream<BandEvent> run(BandLink link) async* {
    await for (final (_, value) in link.notify(kSmaq2ossNotifyChar)) {
    ...
    // No OffloadCheckpoint: this file never asks the watch for its stored
    // history, so there is nothing to tell it to forget.
  }
}

/// The single instance. Const, so it costs nothing to reference.
const Smaq2ossAdapter kSmaq2ossAdapter = Smaq2ossAdapter();
