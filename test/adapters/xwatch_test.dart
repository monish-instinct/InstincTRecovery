...
final Uint8List kSomeFrame = Uint8List.fromList([0xab, 0x01, 0x02, 0x03]);
...
Future<List<BandEvent>> replay(List<(int, Uint8List)> arrivals) async {
  ...
  final sub =
      kXWatchAdapter.run(link).listen(events.add, onDone: done.complete);
  for (final (sec, value) in arrivals) {
    link.feed(kXWatchNotifyChar, value, atSec: sec);
  }
  await link.close();
  await done.future;
  await sub.cancel();
...
void main() {
  test('declares no signals — no card may ever key off this adapter', () {
    expect(kXWatchAdapter.signals, isEmpty);
    expect(kXWatchAdapter.entry.isFramed, isFalse);
  });
  ...
  test('never writes to the peripheral', () async {
    final link = ReplayBandLink();
    final sub = kXWatchAdapter.run(link).listen((_) {});
    link.feed(kXWatchNotifyChar, kSomeFrame, atSec: 1_800_000_000);
    await link.close();
    await sub.cancel();
